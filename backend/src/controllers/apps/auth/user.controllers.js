...
// New phone-based registration
const registerUserWithPhone = asyncHandler(async (req, res) => {
  const { phoneNumber, username, password, role } = req.body;

  // Validate phone number format (basic validation)
  const phoneRegex = /^\+[1-9]\d{1,14}$/;
  if (!phoneRegex.test(phoneNumber)) {
    throw new ApiError(400, "Please provide a valid phone number with country code");
  }
  ...
  if (existedUser) {
    throw new ApiError(409, "User with phone number or username already exists", []);
  }
  ...
  const user = await User.create({
    ...
    loginType: UserLoginType.PHONE_PASSWORD,
    role: role || UserRolesEnum.USER,
  });
  ...
  // For demo purposes, we'll return the OTP in response (DON'T DO THIS IN PRODUCTION)
  console.log(`SMS OTP for ${phoneNumber}: ${otp}`);

  const createdUser = await User.findById(user._id).select(
    "-password -refreshToken -phoneVerificationOTP -phoneVerificationExpiry"
  );
...
const resendPhoneOTP = asyncHandler(async (req, res) => {
  ...
  // Here you would send SMS (integrate with SMS service)
  console.log(`SMS OTP for ${phoneNumber}: ${otp}`);
...
const loginUserWithPhone = asyncHandler(async (req, res) => {
  ...
  if (user.loginType !== UserLoginType.PHONE_PASSWORD) {
    throw new ApiError(
      400,
      "You have previously registered using " +
        user.loginType?.toLowerCase() +
  ...
  if (!user.isPhoneVerified) {
    throw new ApiError(400, "Please verify your phone number before logging in");
  }
...
const updateCurrentUserProfile = asyncHandler(async (req, res) => {
  ...
  if (typeof req.body?.status === "string") {
    updatePayload.status = req.body.status.trim();
  }
  ...
  if (req.file?.filename && user.avatar?.localPath) {
